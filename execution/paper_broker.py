    ...
    def _update_equity(self):
        ...
        if self.active_pool_name == "BINANCE_LIVE_REAL":
            ...
            try:
                ...
                pass
            self.equity = round(self.virtual_cash + crypto_val, 2)
        else:
    ...
    def close_position(self, asset: str, exit_price: float, current_indicators: Optional[dict] = None, sentiment_score: float = 0.5, reason: str = "MANUAL_CLOSE") -> Optional[dict]:
        ...
        if self.active_pool_name in ["BINANCE_LIVE_REAL", "BINANCE_TESTNET_DEMO", "BINANCE_LIVE"] or is_crypto_asset:
            try:
                ...
                # Only attempt real Binance SELL if coins actually exist in wallet
                # If no real coins (paper-only position), close paper record gracefully without exchange call
                if real_token_balance > 0 and close_qty * exit_price >= 1.0:
